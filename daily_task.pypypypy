...
def start_proxy():
    ...
    # 可由 GitHub Actions 通过 PROXY_INDEX 指定起始节点；CF 真人验证后由外层重跑切换节点
    start_index = int(os.getenv("PROXY_INDEX", "0")) % len(nodes)
    ordered_nodes = nodes[start_index:] + nodes[:start_index]
...
async def site_login(page, agent_holder, email, password):
    ...
    if await is_cf_page(page):
        log("⏳ Cloudflare 页面，等待验证...")
        cf_ok = await wait_cf_pass(page, timeout=120)
        if not cf_ok or await is_cf_page(page):
            # CF 真人验证未通过时，不能继续提交登录表单；否则会在无效会话上继续操作
            await save_screenshot(page, "01_cloudflare_blocked")
            log("  ℹ️ 当前节点触发了 Cloudflare 真人验证；本次停止，不继续提交登录")
            return False, "Cloudflare 真人验证未通过或超时"
        await asyncio.sleep(2)
    ...
    # CF 可能在页面初始加载完成后才异步出现；此时也必须换节点
    if await is_cf_page(page):
        log("  ⚠️ 登录提交后触发 Cloudflare 真人验证")
        await save_screenshot(page, "01_cloudflare_after_submit")
        return False, "Cloudflare 真人验证未通过或超时"
    ...
    if await is_hcaptcha_present(page):
        ...
        if ok:
            await asyncio.sleep(2)
            # CF 可能在 hCaptcha 通过后的跳转阶段异步出现
            if await is_cf_page(page):
                await save_screenshot(page, "01_cloudflare_after_captcha")
                log("  ⚠️ hCaptcha 通过后触发 Cloudflare 真人验证")
                return False, "Cloudflare 真人验证未通过或超时"
            try:
                submit = page.locator('button[type="submit"]')
                await submit.first.click()
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            if not ok:
                ...
                # 专用状态码 2：通知 workflow 更换代理节点后重新启动完整流程
                if reason and "Cloudflare" in reason:
                    return 2
                return False
...
def main():
    ...
    # 0=成功，1=普通失败，2=Cloudflare 真人验证触发（通知 workflow 换节点）
    exit(result if isinstance(result, int) else (0 if result else 1))
